...
public class SPopExecutor extends SetExecutor {
  ...
  public void executeCommand(Command command, ExecutionHandlerContext context) {
    ...
    HashSet<Integer> poppedIndices = new HashSet<>();
    List<ByteArrayWrapper> popped = new ArrayList<>();
    try (AutoCloseableLock regionLock = withRegionLock(context, key)) {
      ByteArrayWrapper[]
          entries =
          keyRegion.keySet().toArray(new ByteArrayWrapper[keyRegion.size()]);
      Integer doomed;
      while (poppedIndices.size() < popCount) {
        doomed = rand.nextInt(entries.length);
        if (!poppedIndices.contains(doomed)) {
          poppedIndices.add(doomed);
          ByteArrayWrapper pop = entries[doomed];
          popped.add(pop);
        }
      }
      keyRegion.removeAll(popped);
...
//    respondBulkStrings(command, context, popped);
  }
}
//
//
